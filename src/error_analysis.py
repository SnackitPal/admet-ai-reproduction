...
def error_analysis():
    ...
    for index, row in false_positives.iterrows():
        ...
        Draw.MolToFile(mol, f"images/fp_{index}.png")

    for index, row in false_negatives.iterrows():
        mol = Chem.MolFromSmiles(row['Drug'])
        Draw.MolToFile(mol, f"images/fn_{index}.png")

    print("\nImages of the top 5 false positives and false negatives have been saved to the 'images/' directory.")
